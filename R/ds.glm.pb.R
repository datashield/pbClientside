ds.glm.pb <- function(formula=NULL, data=NULL, family=NULL, offset=NULL, weights=NULL, checks=FALSE, maxit=15, CI=0.95, viewIter=FALSE, datasources=NULL) {
...
#IDENTIFY THE CORRECT DIMENSION FOR START BETAs VIA CALLING FIRST COMPONENT OF glmDS
   ...
   cally1 <- call('glmDS1.pb', formula, family, data)
...
#NOW CALL SECOND COMPONENT OF glmDS TO GENERATE SCORE VECTORS AND INFORMATION MATRICES
    cally2 <- call('glmDS2.pb', formula, family, beta.vect=beta.vect.temp, offset, weights, data)
  ...
  if(converge.state)
  {
    ...
    glmds <- list(
      ...
      dev=dev.total,
      nsubs=nsubs.total,
      AIC=dev.total+2*(length(beta.vect.next)),
